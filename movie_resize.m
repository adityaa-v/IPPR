...
folder = fileparts(which('StillHuman.mp4')); % Determine where demo folder is (works with all versions).
% Pick one of the two demo movies shipped with the Image Processing Toolbox.
% Comment out the other one.
inputFullFileName = fullfile(folder, 'StillHuman.mp4');
...
outputFullFileName = fullfile(pwd, 'StillHuman.Mp4');
...
% Loop throgh the movie, writing all frames out.
for frame =  numberOfFrames
